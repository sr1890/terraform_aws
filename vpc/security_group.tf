...
resource "aws_security_group" "bastion_ssh_all_sg_neo" {

  name        = "bastion_ssh_all_sg_neo"
  ...
  tags = {
    ...
    Name = "Bastion Allow All_Neo"
  }

}

output "output_bastion_ssh" {
  value = aws_security_group.bastion_ssh_all_sg_neo.id
...
resource "aws_security_group" "bastion_ssh_from_nat" {
  ...
  ingress {
    ...
    protocol  = "tcp"
    security_groups = [
      aws_security_group.bastion_ssh_all_sg_neo.id,
